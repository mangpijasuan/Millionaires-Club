...
import { 
  ...
  Menu, Calculator, X, Edit2, Save, Sparkles, Heart, Trash2, Database
} from 'lucide-react';
import { Member, Loan, Transaction, CommunicationLog, YearlyContribution, LoanApplication } from './types';
          ...
          <form onSubmit={onLogin} className="space-y-5">
              <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="email" defaultValue="admin@millionairesclub.com" className="w-full pl-10 pr-4 py-3 border border-slate-200 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-slate-500 outline-none bg-white dark:bg-slate-700 text-slate-900 dark:text-white" required />
                  </div>
              </div>
              <div>
...
const BatchUploadModal = ({ showBatchUpload, setShowBatchUpload }: any) => {
  ...
  return (
      <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-800 w-full max-w-lg rounded-2xl shadow-2xl p-6">
              <h3 className="font-bold text-lg mb-4 text-slate-800 dark:text-white">Batch Upload</h3>
              <textarea className="w-full h-48 p-3 border rounded text-xs font-mono dark:bg-slate-700 dark:text-white dark:border-slate-600" placeholder="Name,ID,Email,JoinDate" value={csvText} onChange={e => setCsvText(e.target.value)}/>
              <div className="flex gap-2 mt-4"><button onClick={() => setShowBatchUpload(false)} className="flex-1 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded">Cancel</button><button onClick={() => { /* Handle logic */ setShowBatchUpload(false); }} className="flex-1 py-2 bg-emerald-600 text-white rounded">Upload</button></div>
          </div>
      </div>
  );
};
...
export default function App() {
  ...
  useEffect(() => {
    const syncData = async () => {
      ...
      try {
        ...
        notify("Data synced with Google Sheets", "info");
      } catch (err) {
  ...
  // Note: For a real app, you'd trigger sheetService.update... on specific actions, not just useEffect on state change
  // to avoid spamming the API. But we'll keep localStorage as the primary fast cache.
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    if (members.some(m => m.id === newMember.id)) { notify('Member ID already exists!', 'error'); return; }
  ...
  if (viewMode === 'admin_login') return <AdminLoginPage onLogin={handleAdminLogin} setViewMode={setViewMode} />;
  ...
  return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 font-sans text-slate-900 dark:text-white flex flex-col md:flex-row transition-colors duration-200">
                  ...
                  {activeTab === 'system' && (
                    <div className="space-y-6 animate-in fade-in">
                       <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                          <h3 className="font-bold text-slate-800 dark:text-white mb-2">System Health</h3>
                          <div className="flex items-center gap-2 p-3 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded-lg text-sm font-medium mt-4"><ShieldCheck size={16}/> All systems operational.</div>
                          <div className="mt-4 text-xs text-slate-500">
                              Data Source: {isSheetsConfigured() ? "Google Sheets (Connected)" : "Local Browser Storage (Offline)"}
                          </div>
                       </div>
                    </div>
                  )}
          ...
          <MemberDetailPane 
            ...
            notify={notify}
          />
          <BatchUploadModal showBatchUpload={showBatchUpload} setShowBatchUpload={setShowBatchUpload} />
